...
# Date: 2026-02-09
# Description:
#   Advanced Dendrochronological Analysis Tools.
#   A library of specialised functions for complex dendroclimatic analyses, 
#   including moving window correlations, significance testing, and response functions.
#
#   Key Functions:
#   - daily_response(): Calculates correlations between tree rings and daily climate 
#     data over a moving window.
#   - boot_ci(): Bootstrap confidence intervals for correlation coefficients.
#   - climate_window(): Aggregates climate data into custom temporal windows.
#
#   Usage:
#   - Sourced by core analysis scripts (e.g., in 02_Climate_Correlations).
# ==============================================================================
...
INPUT_CRONO_FILE <- 'PLACEHOLDER/path/to/chronology.txt'
OUTPUT_DIR       <- 'PLACEHOLDER/path/to/output_dir'
ALPHA_LEVEL      <- 0.05

# Selección de cronología
TIPO_CRONO       <- "res"  # "res" o "std"
...
# -------------------------------------------------------------------
nombre_crono_base <- tools::file_path_sans_ext(basename(INPUT_CRONO_FILE))
...
# 3. PREPARACIÓN (Librerías cargadas arriba)
# -------------------------------------------------------------------
...
# -------------------------------------------------------------------
response <- read.table(INPUT_CRONO_FILE, header = TRUE, sep = "\t")
...
alpha_str <- sprintf("alfa%02d", ALPHA_LEVEL * 100)
sufijo_archivo <- paste0(id_analisis, "_", alpha_str)

message(">>> Guardando Heatmap...")
ggsave(
  filename = file.path(OUTPUT_DIR, "plots", paste0("Heatmap_", sufijo_archivo, ".pdf")),
...
write.table(as.data.frame(resultados$calculations), 
            file.path(OUTPUT_DIR, "txt", paste0("Correl_Matrix_", sufijo_archivo, ".txt")), 
...
if (!is.null(resultados$optimised_return)) {
  ...
  pdf(file.path(OUTPUT_DIR, "plots", paste0("Split_Verification_", sufijo_archivo, ".pdf")), width=10, height=5)
...
archivo_reporte <- file.path(OUTPUT_DIR, "txt", paste0("Validation_Stats_", sufijo_archivo, ".txt"))
